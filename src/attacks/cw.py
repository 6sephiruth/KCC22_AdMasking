...
Source:
    https://github.com/cleverhans-lab/cleverhans/blob/master/cleverhans/tf2/attacks/carlini_wagner_l2.py
"""
...
def carlini_wagner_l2(model_fn, x, **kwargs):
    ...
    For more details on the attack and the parameters
...
class CarliniWagnerL2(object):
    def __init__(
        ...
        :param targets: (optional) Tensor with target labels.
    ...
    def attack(self, x):
        ...
        for i in range(0, len(x), self.batch_size):
            y = self.y[i: i + self.batch_size] if self.y is not None else None
            adv_ex[i : i + self.batch_size] = self._attack(
                x[i : i + self.batch_size], y
            ).numpy()
    ...
    def _attack(self, x, y):
        ...
        for outer_step in range(self.binary_search_steps):
            print(('[%d/%d] CW L2 attack binary search step' %
                   (outer_step + 1, self.binary_search_steps)))
            ...
            for iteration in tqdm(range(self.max_iterations), leave=False):
